...
def train(args):
    ...
    if args.do_train:
        train_set = TUCOREGCNDataset(src_file=args.data_dir, save_file=args.data_dir + "/train" + args.encoder_type + ".pkl", max_seq_length=args.max_seq_length, tokenizer=tokenizer, n_class=n_class, encoder_type=args.encoder_type)
        num_train_steps = int(
    ...
    if args.log:
        ...
        wandb.init(project="ERC", entity="james97black", config = wandb.config)
    if args.do_train:
        best_metric = 0
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_loader, desc = 'Iteration')):
                ...
                loss, _ = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_masks, speaker_ids=speaker_ids, labels=label_ids, target_mask= target_mask)
                if args.log:
                   wandb.log({'loss':loss})
                if n_gpu > 1:
                    loss = loss.mean()
            ...
            if args.f1eval:
                if eval_f1 >= best_metric:
                    ...
                    best_metric = eval_f1
            if args.log:
              wandb.log({key: value for key, value in result.items()})
...
def bert_args(bert_config_file, attn_dropout, hidden_dropout):
...
def check_scores(score_json_path, model_name, hyperparams, eval_f1, test_f1):
    '''Updates score json file if best scores are beaten for a model type'''
    
    with open(score_json_path) as file:
        scores = json.load(file)
        if model_name in scores.keys():
            if eval_f1 > scores[model_name]['eval_f1']:
              scores[model_name]['hyperparams'] = hyperparams
              scores[model_name]['eval_f1'] = eval_f1
              scores[model_name]['test_f1'] = test_f1
              print(f'NEW BEST EVAL F1: {eval_f1}')
              with open(score_json_path, 'w') as f:
                  json.dump(scores, f)
        else:
            scores[model_name] = {'hyperparams':hyperparams, 'eval_f1':eval_f1, 'test_f1':test_f1}
            with open(score_json_path, 'w') as f:
                  json.dump(scores, f)
...
def main():
    ...
    parser.add_argument('--train_batch_size_range', nargs = '+', help='<Required> Set flag', required=True, type = float)  
    parser.add_argument('--learning_rate_range', nargs = '+', help='<Required> Set flag', required=True, type = float)  
    parser.add_argument('--hidden_dropout_prob_range',  nargs = '+', help='<Required> Set flag', required=True, type = float)
    parser.add_argument('--attention_probs_dropout_prob_range',nargs = '+', help='<Required> Set flag', required=True,type = float)
    ...
    for index, param_set in enumerate(param_grid):
        ...
        args.learning_rate = param_set[1]
        args.hidden_dropout_prob = param_set[2]
        args.attention_probs_dropout_prob = param_set[3]
        search_params_run = {re.sub('_range', '', param) : val for param, val in zip(search_params_range.keys(), param_set)}
